...
FFireResult UCombatResolver::ResolveFire(AAShip* Shooter, FIntPoint TargetCell)
{
	...
	if (TargetShip)
	{
		...
		Result.bShipDestroyed = !TargetShip->IsAlive();
		return Result;
	}
...
void UCombatResolver::ApplyDamageToShip(AAShip* Target, int32 Damage)
{
	...
	if (GEngine)
	{
		FString text = FString::Printf(TEXT("ApplyDamage: %s reçoit %d dégâts → pv restant = %d"),
			*Target->GetName(), Damage, Target->GetCurrentHP());
